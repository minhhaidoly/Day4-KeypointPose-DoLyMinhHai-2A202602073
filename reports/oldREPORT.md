...
| Số skeleton | Số `47` (lấy từ dòng đầu tiên). |
| v=2 / v=1 / v=0 | `612 / 143 / 44` (lấy từ dòng Tổng). |
| Thời gian trung bình mỗi ảnh | `16.8` (lấy từ dòng đầu tiên). |
...
OKS trung bình : 0.826
OKS@0.50       : 0.966  (tỉ lệ người gán đúng ở mức 'nhận ra pose')
OKS@0.75       : 0.793  (tỉ lệ người gán đủ chính xác để train)
...
| Ô | Lấy dòng nào |
| --- | --- |
| OKS trung bình | `OKS trung bình : 0.826` → **0.826** |
| OKS@0.50 | `OKS@0.50 : 0.966` → **0.966** |
| OKS@0.75 | `OKS@0.75 : 0.793` → **0.793** |
| Lỗi `dao_trai_phai` | `2` |
| Lỗi `nham_nguoi` | `1` |
| Lỗi `xoa_khop_bi_che` | `5` |
...
pose_mAP50                 0.4520           0.4208    -0.0312
pose_mAP50_95           0.3000           0.2900    -0.0100
pose_precision          0.6000           0.5800    -0.0200
pose_recall             0.5000           0.4800    -0.0200
box_mAP50-95            0.7000           0.6900    -0.0100
```
...
Trả lời tại đây:
`box_mAP50-95 : 0.7000` → **0.7000** | `pose_mAP50-95 : 0.3000` → **0.3000** | chênh 0.4000
...
- [X] Mọi ô số đều có số, không còn trống
- [X] Ba khớp mục 1 khớp với `reports/visibility_report.md`
- [X] Mục 2 có **hai** cột (nếu bạn có rework)
- [X] Ba dòng sửa có tên ảnh
- [X] Mục 3 có tên bạn + 2 khớp lệch
